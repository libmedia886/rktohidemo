...
`TRANSFORM` 单独模式使用实时摄像头输入，走 `VI -> VPSS(4路同源同尺寸NV12) -> RAW/3x TRANSFORM -> VMIX -> OSD -> VO` bind 链路，同屏展示 RAW、UNDISTORT、ROTATE ZOOM、PERSPECTIVE 四路画面；ROTATE ZOOM 路实时更新 XY LUT，VMIX 后的 OSD 层显示中文数据流、四路帧计数、LUT 更新序号和 CPU/GPU/RGA 占用率，并保存 `vo_captures/transform_vo_*.bmp` 供复看。
...
`OSD` 单独模式使用实时摄像头输入，走 `VI -> OSD -> VMIX -> OSD -> VO` bind 链路；第一级 live OSD 在视频上叠加动态目标框、扫描线、状态条和告警块，第二级页面 OSD 叠加中文数据流、参数说明和运行指标。页面展示 region 坐标、zorder 层级、alpha 透明度、enabled 告警开关和两级 OSD 帧计数，并保存 `vo_captures/osd_vo_*.bmp` 供复看。
`RGA` 单独模式使用实时摄像头输入，走 `VI -> RGA -> VMIX -> OSD -> VO` bind 链路，页面会动态轮播 COPY、移动 CROP+SCALE、水平/垂直翻转和 90/180/270 度旋转；屏幕显示当前 RGA 硬件操作、真实数据流、RGA 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/rga_vo_*.bmp` 供复看。
`RESIZE_RGA` 单独模式使用实时摄像头输入，走 `VI -> RESIZE_RGA -> VMIX -> OSD -> VO` bind 链路，页面动态移动裁剪框并改变 crop 尺寸，展示裁剪区域被硬件缩放放大的效果；屏幕显示 crop x/y/w/h、zoom 倍率、RESIZE_RGA 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/resize_rga_vo_*.bmp` 供复看。
`CSC_RGA` 单独模式使用实时摄像头输入，走 `VI -> CSC_RGA(NV12->ARGB8888) -> CSC_RGA(ARGB8888->NV12) -> VMIX -> OSD -> VO` bind 链路，页面显示颜色格式转换流程、动态通道条和两级 CSC 帧计数。
`CSC_CL` 单独模式使用实时摄像头输入，走 `VI -> CSC_CL(NV12->ARGB8888) -> OSD(ARGB8888) -> RGA(960x960) -> VO` bind 链路，页面用大号数据流展示 OpenCL/GPU 颜色矩阵转换，并显示 kernel/queue 耗时、CSC_CL/OSD/RGA 帧计数和 CPU/GPU/RGA 指标；运行时保存 `vo_captures/csc_cl_vo_*.bmp` 供复看。
`CAP_DEHAZE` 单独模式使用实时摄像头输入，程序把 VI 的 NV12 帧转换为 RGB 后送入 CAP_DEHAZE，主画面上下等尺寸对比原始 VI 输入和 CAP 去雾输出；屏幕显示中文数据流、guided radius、t0、beta 参数、CAP_DEHAZE 帧计数和实测 CPU/GPU/RGA 指标，并保存 `vo_captures/cap_dehaze_vo_*.bmp` 供复看。
`DCP_FAST_DEHAZE` 单独模式使用实时摄像头输入，程序把 VI 的 NV12 帧转换为 RGB 后送入 DCP_FAST_DEHAZE，主画面上下等尺寸对比原始 VI 输入和暗通道先验去雾输出，并在屏幕上显示中文数据流、参数、为什么这样做和实测 FPS/CPU/GPU/RGA 指标。
...
`CONV_CL` 单独模式使用实时摄像头输入，程序把 VI 的 NV12 帧转换为 RGBA staging 后送入单个 CONV_CL 组，按顺序加载 SHARPEN、EDGE、EMBOSS、BLUR 四个 3x3 卷积核，再把四个真实 OpenCL 输出合成为 2x2 页面送 VO；屏幕显示中文数据流、四个卷积核帧计数、CPU/GPU/RGA 占用率和 OpenCL kernel/queue 耗时，并保存 `vo_captures/conv_cl_vo_*.bmp` 供复看。
`VMIX` 单独模式使用实时摄像头输入，走 `VI -> VPSS(4路480x480同源输入) -> VMIX(input0..3) -> OSD -> VO` bind 链路；VMIX 页面把四路输入做位置叠放和 alpha 透明混合，屏幕上显示中文数据流、各路 alpha、VI/VMIX 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/vmix_vo_*.bmp` 供复看。
`VPSS` 单独模式使用实时摄像头输入，走 `VI -> VPSS(4路输出) -> VMIX -> OSD -> VO` bind 链路，并在同屏展示 VPSS 多输出能力：全幅缩放、动态裁剪后缩放、水平/垂直翻转切换、中心缩放变化；页面保存 `vo_captures/vpss_vo_*.bmp` 供复看。
`CLAHE` 单独模式使用实时摄像头输入，走 `VI -> VPSS` 分成两路：`VPSS(output0) -> VMIX_RGA(input0)` 显示原始输入，`VPSS(output1) -> CLAHE -> VMIX_RGA(input1) -> OSD -> VO` 显示增强输出；屏幕用上下等尺寸视窗展示原图和局部对比度增强结果，动态展示 clip limit、8x8 tile grid、CLAHE 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/clahe_vo_*.bmp` 供复看。
`RETINEX` 单独模式使用实时摄像头输入，走 `VI -> VPSS` 分成两路：`VPSS(output0) -> VMIX_RGA(input0)` 显示原始输入，`VPSS(output1) -> RETINEX -> VMIX_RGA(input1) -> OSD -> VO` 显示光照校正输出；屏幕用上下等尺寸视窗展示原图和 Retinex 增强结果，显示 gain、threshold、log range、RETINEX 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/retinex_vo_*.bmp` 供复看。
...
当前 `--only VI` 已恢复为原稳定输入分辨率，走 `VI 640x640 -> VMIX(NV12) -> OSD -> VO` bind 显示链路，不再 CPU 拷贝 VI 图像帧；屏幕下方会显示数据流 bind 面板，直接列出谁绑定到谁，例如 `VI0.output -> VMIX80.input0`、`VMIX80.output0 -> OSD81.input`、`OSD81.output0 -> VO0.input0`。端口名由程序在 `output0/output` 和 `input0/input` 中自动探测，绑定成功后按实际命中的端口显示，运行时保存 `vo_captures/vi_vo_*.bmp` 供复看。4K/1080P 大分辨率路径已记录为待验证，不再影响默认 VI 页显示。
实现方式在 `src/alldemo.c`：
