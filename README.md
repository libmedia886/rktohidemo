...
`OSD` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> RESIZE_RGA 640x640 -> OSD -> VMIX -> OSD -> VO` bind 链路；第一级 live OSD 在缩放后的工作帧上叠加动态目标框、扫描线、状态条和告警块，第二级页面 OSD 叠加中文数据流、参数说明和运行指标。页面展示 region 坐标、zorder 层级、alpha 透明度、enabled 告警开关和两级 OSD 帧计数，并保存 `vo_captures/osd_vo_*.bmp` 供复看。
`RGA` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> RGA 640x640 -> VMIX -> OSD -> VO` bind 链路，页面会动态轮播 COPY、移动 CROP+SCALE、水平/垂直翻转和 90/180/270 度旋转；屏幕显示当前 RGA 硬件操作、真实数据流、RGA 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/rga_vo_*.bmp` 供复看。
`RESIZE_RGA` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> RESIZE_RGA 640x640 -> VMIX -> OSD -> VO` bind 链路，页面动态移动裁剪框并改变 crop 尺寸，展示裁剪区域被硬件缩放放大的效果；屏幕显示 crop x/y/w/h、zoom 倍率、RESIZE_RGA 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/resize_rga_vo_*.bmp` 供复看。
`CSC_RGA` 单独模式使用实时摄像头输入，走 `VI -> CSC_RGA(NV12->ARGB8888) -> CSC_RGA(ARGB8888->NV12) -> VMIX -> OSD -> VO` bind 链路，页面显示颜色格式转换流程、动态通道条和两级 CSC 帧计数。
`CSC_CL` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> RESIZE_RGA 640x640 -> CSC_CL(NV12->ARGB8888) -> OSD(ARGB8888) -> RGA(960x960) -> VO` bind 链路，页面用大号数据流展示 OpenCL/GPU 颜色矩阵转换，并显示 kernel/queue 耗时、CSC_CL/OSD/RGA 帧计数和 CPU/GPU/RGA 指标；运行时保存 `vo_captures/csc_cl_vo_*.bmp` 供复看。
`CAP_DEHAZE` 单独模式使用 3840x2160 实时摄像头输入，页面侧先把 VI 的 NV12 帧下采样到处理尺寸并转换为 RGB 后送入 CAP_DEHAZE，主画面上下等尺寸对比原始 VI 输入和 CAP 去雾输出；屏幕显示中文数据流、guided radius、t0、beta 参数、CAP_DEHAZE 帧计数和实测 CPU/GPU/RGA 指标，并保存 `vo_captures/cap_dehaze_vo_*.bmp` 供复看。
`DCP_FAST_DEHAZE` 单独模式使用实时摄像头输入，程序把 VI 的 NV12 帧转换为 RGB 后送入 DCP_FAST_DEHAZE，主画面上下等尺寸对比原始 VI 输入和暗通道先验去雾输出，并在屏幕上显示中文数据流、参数、为什么这样做和实测 FPS/CPU/GPU/RGA 指标。DCP_FAST_DEHAZE 尚未纳入本轮 4K 输入通过列表，需先修复输出 buffer 归还和帧计数验证。
...
`CLAHE` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> VPSS` 分成两路 640x640：`VPSS(output0) -> VMIX_RGA(input0)` 显示原始输入，`VPSS(output1) -> CLAHE -> VMIX_RGA(input1) -> OSD -> VO` 显示增强输出；屏幕用上下等尺寸视窗展示原图和局部对比度增强结果，动态展示 clip limit、8x8 tile grid、CLAHE 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/clahe_vo_*.bmp` 供复看。
`RETINEX` 单独模式使用 3840x2160 实时摄像头输入，走 `VI 3840x2160 -> VPSS` 分成两路 640x640：`VPSS(output0) -> VMIX_RGA(input0)` 显示原始输入，`VPSS(output1) -> RETINEX -> VMIX_RGA(input1) -> OSD -> VO` 显示光照校正输出；屏幕用上下等尺寸视窗展示原图和 Retinex 增强结果，显示 gain、threshold、log range、RETINEX 帧计数和 CPU/GPU/RGA 指标，并保存 `vo_captures/retinex_vo_*.bmp` 供复看。
