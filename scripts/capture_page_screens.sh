...
if [ "${#PAGES[@]}" -eq 0 ]; then
  PAGES=(
    ...
    RETINEX RETINEX_OFFLINE TNR_CL HIGHLIGHT_SUPPRESS HIGHLIGHT_SUPPRESS_VI
    ...
    DUALVIEW STEREO_3D PANO AVM AVM2D SVM3D DETECT_NPU VENC VDEC RTSP_SEND RTSP_RECV
    PIC_IO LICENSE
  )
fi
