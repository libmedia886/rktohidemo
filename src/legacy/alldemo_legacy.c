...
#include "app/loop_player.h"
#include "app/page_registry.h"
#include "app/page_runtime.h"
...
#define HIGHLIGHT_SUPPRESS_OUTPUT_POOL CONV_CL_OUTPUT_POOL
...
#define LIVE_HIGHLIGHT_SUPPRESS_GRP 120
...
#define HIGHLIGHT_SUPPRESS_THRESHOLD_LOW 0.58f
#define HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH 0.86f
#define HIGHLIGHT_SUPPRESS_KNEE 0.68f
#define HIGHLIGHT_SUPPRESS_RATIO 0.08f
#define HIGHLIGHT_SUPPRESS_STRENGTH 0.94f
#define HIGHLIGHT_SUPPRESS_CHROMA_LOW 0.05f
#define HIGHLIGHT_SUPPRESS_CHROMA_HIGH 0.52f
...
static const char *g_bind_highlight_suppress_in_port = NULL;
static const char *g_bind_highlight_suppress_src_port = NULL;
static const char *g_highlight_suppress_mix_module = "VMIX";
...
static void draw_highlight_suppress_showcase(uint8_t *dst, int stride, int x, int y, int w, int h,
                                             const uint8_t *input_nv12,
                                             const uint8_t *output_nv12) {
    char runtime[128];
    char param_line[192];
    ...
    format_runtime_status(runtime, sizeof(runtime));
    snprintf(param_line, sizeof(param_line),
             "PARAM low=%.2f high=%.2f knee=%.2f ratio=%.2f strength=%.2f",
             HIGHLIGHT_SUPPRESS_THRESHOLD_LOW, HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH,
             HIGHLIGHT_SUPPRESS_KNEE, HIGHLIGHT_SUPPRESS_RATIO,
             HIGHLIGHT_SUPPRESS_STRENGTH);

    fill_rect_nv12(dst, stride, x, y, w, top_h, 6, 13, 24);
    stroke_rect_nv12(dst, stride, x, y, w, top_h, 2, 66, 150, 210);
    draw_utf8_text(dst, stride, x + 24, y + 18,
                   "HIGHLIGHT_SUPPRESS：NV12高光软压制", 28, 170, 255, 220);
    draw_utf8_text(dst, stride, x + 24, y + 62,
                   "数据流：合成NV12强反光场景 -> 高光soft-knee压缩 -> 输入/输出上下对比。",
                   20, 210, 235, 255);
    draw_utf8_text(dst, stride, x + 24, y + 96,
                   "为什么这样做：只压低接近白色的刺眼亮部，尽量保留普通高亮和有色区域。",
                   20, 255, 230, 120);
    draw_utf8_text(dst, stride, x + 24, y + 130,
                   "当前模块为OpenCL/EGL GPU路径；CPU/LUT仅作为fallback。",
                   20, 255, 230, 120);

    draw_nv12_comparison(dst, stride, x, compare_y, w, compare_h,
                         input_nv12, output_nv12,
                         "TOP: NV12 GLARE INPUT", "BOTTOM: SOFT-KNEE OUTPUT");

    fill_rect_nv12(dst, stride, x, bottom_y, w, bottom_h, 5, 10, 18);
    stroke_rect_nv12(dst, stride, x, bottom_y, w, bottom_h, 2, 0, 190, 170);
    draw_utf8_text(dst, stride, x + 24, bottom_y + 22,
                   "展示重点：观察白色反光斑是否变柔和，背景层次和有色目标是否保持自然。",
                   22, 170, 255, 220);
    draw_utf8_text(dst, stride, x + 24, bottom_y + 66,
                   "算法边界：这是实时亮度压制，不做RAW域恢复，也不重建过曝丢失纹理。",
                   20, 210, 235, 255);
    draw_text(dst, stride, x + 24, bottom_y + 110, param_line, 1, 190, 230, 255);
    draw_text(dst, stride, x + 24, bottom_y + 144, runtime, 2, 255, 230, 120);
}
...
static void draw_effect_tile(uint8_t *dst, int stride, int x, int y, int w, int h,
    ...
    } else if (strcmp(g_tiles[idx].name, "HIGHLIGHT_SUPPRESS") == 0 && (retinex_in || retinex_live)) {
        draw_nv12_comparison(dst, stride, x + 6, y + 30, w - 12, h - 38,
                             retinex_in, retinex_live, "GLARE IN", "SOFT-KNEE");
...
static void draw_tile_content(uint8_t *dst, int stride, int x, int y, int w, int h,
    ...
    if (strcmp(g_tiles[idx].name, "HIGHLIGHT_SUPPRESS") == 0 && (cam || retinex_live)) {
        draw_highlight_suppress_showcase(dst, stride, x + 12, y + 12, w - 24, h - 24,
                                         cam, retinex_live);
        return;
    }
...
static int bind_vi_highlight_suppress_vmix_osd_vo(void) {
    const char *mix = g_highlight_suppress_mix_module ? g_highlight_suppress_mix_module : "VMIX";
    const char *out_ports[] = {"output0", "output"};
    const char *vi_out_ports[] = {"output", "output0"};
    const char *hs_in_ports[] = {"input", "input0"};
    const char *vmix_ports[] = {"input0", "input"};
    const char *vo_in_ports[] = {"input0", "input"};
    const char *osd_in_ports[] = {"input", "input0"};
    g_bind_vi_src_port = NULL;
    g_bind_compare_effect_resize_in_port = NULL;
    g_bind_compare_effect_resize_src_port = NULL;
    g_bind_highlight_suppress_in_port = NULL;
    g_bind_highlight_suppress_src_port = NULL;
    g_bind_vmix_in_port = NULL;
    g_bind_vmix_src_port = NULL;
    g_bind_osd_in_port = NULL;
    g_bind_osd_src_port = NULL;
    ...
    if (bind_first_match("VI", 0, vi_out_ports, (int)ARRAY_SIZE(vi_out_ports),
                         "HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         hs_in_ports, (int)ARRAY_SIZE(hs_in_ports),
                         &g_bind_vi_src_port, &g_bind_highlight_suppress_in_port) != 0) {
        fprintf(stderr, "bind failed: VI -> HIGHLIGHT_SUPPRESS\n");
        return -1;
    }
    if (bind_first_match("HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         out_ports, (int)ARRAY_SIZE(out_ports),
                         "RESIZE_RGA", LIVE_COMPARE_EFFECT_RESIZE_GRP,
                         hs_in_ports, (int)ARRAY_SIZE(hs_in_ports),
                         &g_bind_highlight_suppress_src_port,
                         &g_bind_compare_effect_resize_in_port) != 0) {
        fprintf(stderr, "bind failed: HIGHLIGHT_SUPPRESS -> display resize\n");
        MEDIA_SYS_UnBind("VI", 0, g_bind_vi_src_port,
                         "HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         g_bind_highlight_suppress_in_port);
        return -1;
    }
    if (bind_first_match("RESIZE_RGA", LIVE_COMPARE_EFFECT_RESIZE_GRP,
                         out_ports, (int)ARRAY_SIZE(out_ports),
                         mix, DISPLAY_VMIX_GRP, vmix_ports, (int)ARRAY_SIZE(vmix_ports),
                         &g_bind_compare_effect_resize_src_port, &g_bind_vmix_in_port) != 0) {
        fprintf(stderr, "bind failed: HIGHLIGHT_SUPPRESS display resize -> %s\n", mix);
        MEDIA_SYS_UnBind("HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         g_bind_highlight_suppress_src_port,
                         "RESIZE_RGA", LIVE_COMPARE_EFFECT_RESIZE_GRP,
                         g_bind_compare_effect_resize_in_port);
        MEDIA_SYS_UnBind("VI", 0, g_bind_vi_src_port,
...
static void unbind_vi_highlight_suppress_vmix_osd_vo(int enabled) {
    const char *mix = g_highlight_suppress_mix_module ? g_highlight_suppress_mix_module : "VMIX";
    if (!enabled) return;
    if (g_bind_osd_src_port && g_bind_vo_in_port) {
        MEDIA_SYS_UnBind("OSD", DISPLAY_OSD_GRP, g_bind_osd_src_port,
    ...
    if (g_bind_highlight_suppress_src_port && g_bind_compare_effect_resize_in_port) {
        MEDIA_SYS_UnBind("HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         g_bind_highlight_suppress_src_port,
                         "RESIZE_RGA", LIVE_COMPARE_EFFECT_RESIZE_GRP,
                         g_bind_compare_effect_resize_in_port);
    }
    if (g_bind_vi_src_port && g_bind_highlight_suppress_in_port) {
        MEDIA_SYS_UnBind("VI", 0, g_bind_vi_src_port,
                         "HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP,
                         g_bind_highlight_suppress_in_port);
    }
...
static void draw_nv12_circle_frame(uint8_t *dst, int frame_w, int frame_h, int stride,
    ...
    for (int y = cy - radius; y <= cy + radius; ++y) {
        ...
        uint8_t *yp = dst + (size_t)y * stride;
    ...
    uint8_t *uv = dst + (size_t)stride * frame_h;
    for (int y = (cy - radius) / 2; y <= (cy + radius) / 2; ++y) {
        if (y < 0 || y >= frame_h / 2) continue;
        for (int x = (cx - radius) & ~1; x <= (cx + radius); x += 2) {
            if (x < 0 || x + 1 >= frame_w) continue;
            int dx = x + 1 - cx;
            int dy = y * 2 + 1 - cy;
            if (dx * dx + dy * dy > r2) continue;
            uint8_t *p = uv + (size_t)y * stride + x;
            p[0] = uu;
            p[1] = vv;
        }
    }
...
static void fill_highlight_demo_nv12(uint8_t *dst, int frame) {
    fill_rect_nv12_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 0, 0, CAM_W, CAM_H, 30, 42, 48);
    for (int y = 0; y < CAM_H; y += 40) {
        uint8_t shade = (uint8_t)(42 + (y * 64) / CAM_H);
        fill_rect_nv12_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 0, y, CAM_W, 20,
                             shade, (uint8_t)(shade + 18), (uint8_t)(shade + 28));
    }
    fill_rect_nv12_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 72, 118, 240, 330, 50, 86, 110);
    fill_rect_nv12_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 350, 172, 190, 260, 110, 70, 48);
    fill_rect_nv12_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 90, 470, 460, 54, 68, 92, 102);
    for (int i = 0; i < 5; ++i) {
        int x = 130 + i * 86 + ((frame * 3 + i * 17) % 18);
        int y = 218 + ((i * 41 + frame * 2) % 74);
        draw_nv12_circle_frame(dst, CAM_W, CAM_H, CAM_STRIDE, x, y, 44 + i * 4,
                               255, 255, 246, 22);
    }
    draw_nv12_circle_frame(dst, CAM_W, CAM_H, CAM_STRIDE,
                           430 + ((frame * 2) % 34), 242, 96,
                           255, 255, 255, 42);
    draw_nv12_circle_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 210, 362, 64,
                           255, 246, 224, 24);
    draw_nv12_circle_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 116, 180, 58,
                           255, 255, 255, 28);
    draw_nv12_circle_frame(dst, CAM_W, CAM_H, CAM_STRIDE, 520, 492, 34,
                           238, 170, 70, 18);
}

static int setup_live_highlight_suppress(void) {
    if (MEDIA_POOL_Create(OSD_INPUT_POOL, CAM_FRAME_SIZE, 4) != 0) return -1;
    if (MEDIA_POOL_Create(OSD_OUTPUT_POOL, CAM_FRAME_SIZE, 4) != 0) {
        MEDIA_POOL_Destroy(OSD_INPUT_POOL);
        return -1;
    ...
    attr.input_stride = CAM_STRIDE;
    attr.output_stride = CAM_STRIDE;
    attr.threshold_low = HIGHLIGHT_SUPPRESS_THRESHOLD_LOW;
    attr.threshold_high = HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH;
    attr.knee = HIGHLIGHT_SUPPRESS_KNEE;
    attr.ratio = HIGHLIGHT_SUPPRESS_RATIO;
    attr.strength = HIGHLIGHT_SUPPRESS_STRENGTH;
    attr.chroma_low = HIGHLIGHT_SUPPRESS_CHROMA_LOW;
    attr.chroma_high = HIGHLIGHT_SUPPRESS_CHROMA_HIGH;

    if (MEDIA_HIGHLIGHT_SUPPRESS_CreateGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP, &attr) != 0 ||
        MEDIA_HIGHLIGHT_SUPPRESS_Start(LIVE_HIGHLIGHT_SUPPRESS_GRP) != 0) {
        MEDIA_HIGHLIGHT_SUPPRESS_DestroyGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP);
        MEDIA_POOL_Destroy(OSD_OUTPUT_POOL);
        MEDIA_POOL_Destroy(OSD_INPUT_POOL);
        return -1;
    }
...
static void cleanup_live_highlight_suppress(int enabled) {
    if (!enabled) return;
    MEDIA_HIGHLIGHT_SUPPRESS_Stop(LIVE_HIGHLIGHT_SUPPRESS_GRP);
    MEDIA_HIGHLIGHT_SUPPRESS_DestroyGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP);
    MEDIA_POOL_Destroy(OSD_OUTPUT_POOL);
    MEDIA_POOL_Destroy(OSD_INPUT_POOL);
}

static int setup_live_highlight_suppress_bind(void) {
    if (MEDIA_POOL_Create(HIGHLIGHT_SUPPRESS_OUTPUT_POOL, live_camera_input_frame_size(), 4) != 0) {
        return -1;
    }
    ...
    attr.format = MEDIA_FORMAT_NV12;
    attr.input_depth = 4;
    attr.output_pool_id = HIGHLIGHT_SUPPRESS_OUTPUT_POOL;
    attr.input_stride = g_camera_input_stride;
    attr.output_stride = g_camera_input_stride;
    attr.threshold_low = HIGHLIGHT_SUPPRESS_THRESHOLD_LOW;
    attr.threshold_high = HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH;
    ...
    if (MEDIA_HIGHLIGHT_SUPPRESS_CreateGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP, &attr) != 0 ||
        MEDIA_HIGHLIGHT_SUPPRESS_Start(LIVE_HIGHLIGHT_SUPPRESS_GRP) != 0) {
        MEDIA_HIGHLIGHT_SUPPRESS_DestroyGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP);
        MEDIA_POOL_Destroy(HIGHLIGHT_SUPPRESS_OUTPUT_POOL);
        return -1;
    }
...
static void cleanup_live_highlight_suppress_bind(int enabled) {
    if (!enabled) return;
    MEDIA_HIGHLIGHT_SUPPRESS_Stop(LIVE_HIGHLIGHT_SUPPRESS_GRP);
    MEDIA_HIGHLIGHT_SUPPRESS_DestroyGrp(LIVE_HIGHLIGHT_SUPPRESS_GRP);
    MEDIA_POOL_Destroy(HIGHLIGHT_SUPPRESS_OUTPUT_POOL);
...
static int process_live_highlight_suppress(const uint8_t *src, uint8_t *dst) {
    MEDIA_BUFFER out = {-1, -1};
    int ret = -1;
    if (send_copied_frame("HIGHLIGHT_SUPPRESS", LIVE_HIGHLIGHT_SUPPRESS_GRP, "input",
                          OSD_INPUT_POOL, src, CAM_FRAME_SIZE, 20) != 0) {
        return -1;
    }
    if (MEDIA_HIGHLIGHT_SUPPRESS_GetFrame(LIVE_HIGHLIGHT_SUPPRESS_GRP, &out, 1000) == 0) {
        ret = copy_from_buffer(out, dst, CAM_FRAME_SIZE);
        MEDIA_HIGHLIGHT_SUPPRESS_ReleaseFrame(LIVE_HIGHLIGHT_SUPPRESS_GRP, out);
    }
    return ret;
}
...
static int update_highlight_suppress_vi_overlay(uint64_t vi_count,
                                                uint64_t suppress_count,
                                                uint64_t effect_resize_count,
                                                int passthrough_mode,
                                                const char *module_perf,
                                                int frame) {
    static uint8_t masks[16][1024 * 96];
    const int view_x = (SCREEN_W - COMPARE_VIEW_W) / 2;
    const int view_y = (SCREEN_H - COMPARE_VIEW_H) / 2;
    ...
    snprintf(count_line, sizeof(count_line),
             "FRAMES  VI=%llu  HIGHLIGHT_SUPPRESS=%llu  RESIZE=%llu",
             (unsigned long long)vi_count,
             (unsigned long long)suppress_count,
             (unsigned long long)effect_resize_count);
    snprintf(param_line, sizeof(param_line),
             "LOW %.2f  HIGH %.2f  KNEE %.2f  RATIO %.2f  STRENGTH %.2f",
             HIGHLIGHT_SUPPRESS_THRESHOLD_LOW,
             HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH,
             HIGHLIGHT_SUPPRESS_KNEE,
             HIGHLIGHT_SUPPRESS_RATIO,
             HIGHLIGHT_SUPPRESS_STRENGTH);
    snprintf(mode_line, sizeof(mode_line), "MODE %s  1s AUTO SWITCH  MODULE %s",
             passthrough_mode ? "BYPASS RAW" : "SUPPRESS ON",
             module_perf ? module_perf : "N/A");
    snprintf(bind_line, sizeof(bind_line),
             "VI0.%s -> HIGHLIGHT_SUPPRESS%d.%s -> RESIZE%d.%s -> %s%d.%s -> OSD%d -> VO0",
             g_bind_vi_src_port ? g_bind_vi_src_port : "output",
             LIVE_HIGHLIGHT_SUPPRESS_GRP,
             g_bind_highlight_suppress_in_port ? g_bind_highlight_suppress_in_port : "input",
             LIVE_COMPARE_EFFECT_RESIZE_GRP,
             g_bind_compare_effect_resize_src_port ? g_bind_compare_effect_resize_src_port : "output0",
             mix,
             DISPLAY_VMIX_GRP,
    ...
    if (update_osd_rect_region(18, view_x, view_y - 50, 500, 38, 5, 10, 18, 220) != 0) return -1;
    if (update_osd_utf8_text_region(20, view_x + 16, view_y - 45, 24, 190, 230, 255,
                                    passthrough_mode ? "当前：高光压制BYPASS原图" : "当前：高光压制输出",
                                    masks[0], sizeof(masks[0]), 1024, 96) != 0) return -1;

    if (update_osd_rect_region(22, 34, 1510, 1012, 286, 5, 10, 18, 226) != 0) return -1;
    if (update_osd_utf8_text_region(23, 66, 1536, 34, 160, 255, 220,
                                    "HIGHLIGHT_SUPPRESS_VI：4K实时输入，BYPASS/高光压制交替显示",
                                    masks[2], sizeof(masks[2]), 1024, 96) != 0) return -1;
    if (update_osd_utf8_text_region(24, 68, 1588, 24, 190, 230, 255,
                                    "数据流：VI 3840x2160直接进入HIGHLIGHT_SUPPRESS，最后RESIZE_RGA缩放为1080x608显示。",
                                    masks[3], sizeof(masks[3]), 1024, 96) != 0) return -1;
    if (update_osd_utf8_text_region(25, 68, 1630, 23, 255, 230, 120,
                                    "为什么这样做：只压接近白色的刺眼高光，避免普通亮部和有色目标被整体压暗。",
                                    masks[4], sizeof(masks[4]), 1024, 96) != 0) return -1;

    int pulse_w = passthrough_mode ? 188 : 392 + ((frame * 5) % 90);
    if (update_osd_rect_region(26, 68, 1676, pulse_w, 28,
                               passthrough_mode ? 120 : 255,
                               passthrough_mode ? 220 : 210,
                               passthrough_mode ? 255 : 70,
                               220) != 0) return -1;
    if (update_osd_text_region(27, 548, 1666, 1, 170, 255, 220,
                               param_line, masks[5], sizeof(masks[5])) != 0) return -1;
    if (update_osd_text_region(28, 68, 1716, 1, 170, 255, 220,
...
static void draw_showcase_page_refs(uint8_t *canvas, int stride, int frame, int rotate_main,
    ...
    if (only_tile) {
        ...
        int total_pages = alldemo_default_page_count();
        int page = rotate_main ?
            (frame / (FPS * PAGE_ROTATE_SECONDS)) % total_pages : 0;
        const char *name = alldemo_default_page_name(page);
        if (!name) name = "VI";
...
int alldemo_legacy_main(int argc, char **argv) {
    signal(SIGINT, on_signal);
    signal(SIGTERM, on_signal);
    ...
    int solid_test = 0;
    int rotate_main = 1;
    const demo_loop_t *demo_loop = alldemo_customer_loop();
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(argv[i], "--customer-demo") == 0) demo_loop = alldemo_customer_loop();
        if (strcmp(argv[i], "--engineering-demo") == 0) demo_loop = alldemo_engineering_loop();
    ...
    if (only_tile && (!page_ops_find(only_tile) || find_tile_index(only_tile) < 0)) {
        fprintf(stderr, "unknown tile for --only: %s\n", only_tile);
        return 1;
    }

    const page_ops_t *only_ops = only_tile ? page_ops_find(only_tile) : NULL;
    if (only_ops && only_ops->impl == PAGE_IMPL_STANDALONE && only_ops->run) {
        return only_ops->run(&g_running);
    }

    page_runtime_t page_rt;
    page_runtime_init(&page_rt, only_tile, solid_test);
    int use_4k_camera_input = page_rt.use_4k_camera_input;
    int use_1080p_camera_input = page_rt.use_1080p_camera_input;
    ...
    if (!only_tile && !solid_test && !heavy_probe) {
        return run_default_only_loop(argv[0], rotate_main, demo_loop, &g_running);
    }
    ...
    const int highlight_suppress_only_page = only_tile && strcasecmp(only_tile, "HIGHLIGHT_SUPPRESS") == 0;
    ...
    int live_highlight_suppress_ok = 0;
    if (!solid_test && highlight_suppress_only_page && setup_live_highlight_suppress() == 0) {
        live_highlight_suppress_ok = 1;
    }
    int live_edof_ok = 0;
    ...
    if (MEDIA_POOL_Create(DISPLAY_POOL, display_size, 4) != 0) {
        ...
        cleanup_live_highlight_suppress(live_highlight_suppress_ok);
    ...
    int use_vmix_osd_display = page_rt.use_vmix_osd_display;
    int use_vi_bind_display = page_rt.use_vi_bind_display;
    int use_wbc_bind_display = page_rt.use_wbc_bind_display;
    int use_vmix_bind_display = page_rt.use_vmix_bind_display;
    int use_vpss_bind_display = page_rt.use_vpss_bind_display;
    int use_osd_bind_display = page_rt.use_osd_bind_display;
    int use_clahe_bind_display = page_rt.use_clahe_bind_display;
    int use_retinex_bind_display = page_rt.use_retinex_bind_display;
    int use_highlight_suppress_vi_bind_display = page_rt.use_highlight_suppress_vi_bind_display;
    int use_cap_dehaze_bind_display = page_rt.use_cap_dehaze_bind_display;
    int use_dcp_dehaze_bind_display = page_rt.use_dcp_dehaze_bind_display;
    int use_rga_bind_display = page_rt.use_rga_bind_display;
    int use_resize_bind_display = page_rt.use_resize_bind_display;
    int use_csc_rga_bind_display = page_rt.use_csc_rga_bind_display;
    int use_csc_cl_bind_display = page_rt.use_csc_cl_bind_display;
    int use_conv_cl_bind_display = page_rt.use_conv_cl_bind_display;
    int use_transform_bind_display = page_rt.use_transform_bind_display;
    int use_stereo_bind_display = page_rt.use_stereo_bind_display;
    int use_vi_big_display = page_rt.use_vi_big_display;
    ...
    int highlight_suppress_vi_bind_display_ok = 0;
    ...
    int live_highlight_suppress_bind_ok = 0;
    ...
    int display_vmix_inputs =
        ...
        ((use_wbc_bind_display || use_clahe_bind_display || use_retinex_bind_display ||
          use_highlight_suppress_vi_bind_display) ? 2 : 1);
    ...
    int defer_vmix_osd_for_highlight_suppress = use_highlight_suppress_vi_bind_display;
    ...
    if (use_vmix_osd_display && !defer_vmix_osd_for_csc_cl &&
        ...
        !defer_vmix_osd_for_highlight_suppress &&
        ...
        display_vmix_osd_ok = 1;
               ...
               !defer_vmix_osd_for_retinex && !defer_vmix_osd_for_highlight_suppress &&
               !defer_vmix_osd_for_conv_cl) {
    ...
    if (use_highlight_suppress_vi_bind_display && setup_live_highlight_suppress_bind() == 0) {
        live_highlight_suppress_bind_ok = 1;
        g_highlight_suppress_mix_module = "VMIX_RGA";
        if (use_vmix_osd_display &&
            setup_display_vmix_rga_osd_inputs(dstride, display_size, 1) == 0) {
            display_vmix_osd_ok = 1;
            display_vmix_rga_osd_ok = 1;
        ...
        if (!live_compare_display_resizes_ok) {
            cleanup_live_compare_display_resizes(live_compare_display_resizes_ok);
            live_compare_display_resizes_ok = 0;
            cleanup_live_highlight_suppress_bind(live_highlight_suppress_bind_ok);
            live_highlight_suppress_bind_ok = 0;
            use_vmix_osd_display = 0;
            g_highlight_suppress_mix_module = "VMIX";
        }
    }
    ...
    int camera_ok = 0;
    int need_camera = page_rt.need_camera;
    ...
    if (use_highlight_suppress_vi_bind_display) {
        if (!camera_ok || !live_highlight_suppress_bind_ok || !display_vmix_osd_ok ||
            bind_vi_highlight_suppress_vmix_osd_vo() != 0) {
            fprintf(stderr, "HIGHLIGHT_SUPPRESS_VI bind display setup failed; CPU copy fallback is disabled for --only HIGHLIGHT_SUPPRESS_VI\n");
            if (camera_ok) MEDIA_VI_Disable(0);
            cleanup_live_compare_display_resizes(live_compare_display_resizes_ok);
            live_compare_display_resizes_ok = 0;
            cleanup_live_highlight_suppress_bind(live_highlight_suppress_bind_ok);
        ...
        highlight_suppress_vi_bind_display_ok = 1;
        (void)update_display_osd_text("HIGHLIGHT_SUPPRESS_VI  4K BYPASS/SUPPRESS",
                                      "VI 4K -> HIGHLIGHT_SUPPRESS -> RESIZE -> VO");
        (void)update_highlight_suppress_vi_overlay(0, 0, 0, 1, "N/A", 0);
    }
    if (use_cap_dehaze_bind_display || use_dcp_dehaze_bind_display) {
    ...
    int tnr_frames = 0;
    int highlight_suppress_frames = 0;
    int edof_frames = 0;
    ...
    uint8_t *last_highlight_suppress = malloc(CAM_FRAME_SIZE);
    ...
    if (last_highlight_suppress) memset(last_highlight_suppress, 0, CAM_FRAME_SIZE);
    ...
    while (g_running) {
        if (camera_ok && !vi_bind_display_ok && !vpss_bind_display_ok &&
            ...
            !highlight_suppress_vi_bind_display_ok &&
        ...
        if (live_highlight_suppress_ok && last_cam && last_highlight_suppress) {
            fill_highlight_demo_nv12(last_cam, frame);
            if (process_live_highlight_suppress(last_cam, last_highlight_suppress) == 0) {
                highlight_suppress_frames++;
            }
        }
        ...
        if (highlight_suppress_vi_bind_display_ok) {
            int passthrough_mode = ((frame / FPS) % 2) == 0;
            if ((frame % FPS) == 0) {
                (void)MEDIA_HIGHLIGHT_SUPPRESS_SetPassthrough(LIVE_HIGHLIGHT_SUPPRESS_GRP,
                                                              passthrough_mode);
            }
            if ((frame % 15) == 0) {
                uint64_t vi_count = 0;
                ...
                uint64_t effect_resize_count = 0;
                MEDIA_HIGHLIGHT_SUPPRESS_PERF hs_perf = {0};
                char module_text[48];
                char perf[180];
                char runtime[128];
                char gpu_text[24];
                ...
                (void)MEDIA_SYS_GetModuleFrameCount("HIGHLIGHT_SUPPRESS",
                                                    LIVE_HIGHLIGHT_SUPPRESS_GRP,
                                                    &suppress_count);
                (void)MEDIA_SYS_GetModuleFrameCount("RESIZE_RGA",
                                                    LIVE_COMPARE_EFFECT_RESIZE_GRP,
                                                    &effect_resize_count);
                if (MEDIA_HIGHLIGHT_SUPPRESS_GetLastPerf(LIVE_HIGHLIGHT_SUPPRESS_GRP,
                                                         &hs_perf) == 0) {
                    if (hs_perf.gpu_enabled) {
                        snprintf(module_text, sizeof(module_text), "GPU %.3f/%.3fms",
                                 hs_perf.gpu_kernel_ms, hs_perf.gpu_queue_ms);
                    } else {
                        snprintf(module_text, sizeof(module_text), "CPU %.3fms", hs_perf.cpu_ms);
                    }
                } else {
                    snprintf(module_text, sizeof(module_text), "N/A");
                }
                format_runtime_status(runtime, sizeof(runtime));
                snprintf(perf, sizeof(perf), "P%02d/%02d HS_VI 4K %s OUT %llu RESIZE %llu %s",
                         module_page_number("HIGHLIGHT_SUPPRESS_VI"), module_page_total(),
                         passthrough_mode ? "BYPASS" : "SUPPRESS",
                         (unsigned long long)suppress_count,
                         (unsigned long long)effect_resize_count,
                         runtime);
                (void)update_display_osd_text("HIGHLIGHT_SUPPRESS_VI  4K BYPASS/SUPPRESS",
                                              perf);
                (void)update_highlight_suppress_vi_overlay(vi_count, suppress_count,
                                                           effect_resize_count,
                                                           passthrough_mode,
                                                           module_text,
                                                           frame);
                if ((frame % FPS) == 0) {
                    ...
                    snprintf(rga_text, sizeof(rga_text), g_perf.rga_available ? "%.0f%%" : "N/A",
                             g_perf.rga_percent);
                    printf("HIGHLIGHT_SUPPRESS_VI vi_frames=%llu suppress_frames=%llu resize_frames=%llu passthrough=%d low=%.2f high=%.2f ratio=%.2f strength=%.2f cpu=%.0f%% gpu=%s rga=%s module=%s\n",
                           (unsigned long long)vi_count,
                           (unsigned long long)suppress_count,
                           (unsigned long long)effect_resize_count,
                           passthrough_mode,
        ...
        display_refs_t display_refs = {
            (cam_frames > 0 ||
             ...
             (tnr_cl_only_page && tnr_frames > 0) ||
             (highlight_suppress_only_page && highlight_suppress_frames > 0)) ? last_cam : NULL,
            ...
            highlight_suppress_only_page && highlight_suppress_frames > 0 ? last_highlight_suppress :
        ...
        if (highlight_suppress_only_page && (frame % FPS) == 0) {
            MEDIA_HIGHLIGHT_SUPPRESS_PERF hs_perf = {0};
            char gpu_text[24];
            char rga_text[24];
            char module_text[32];
            ...
            snprintf(rga_text, sizeof(rga_text), g_perf.rga_available ? "%.0f%%" : "N/A",
                     g_perf.rga_percent);
            if (live_highlight_suppress_ok &&
                MEDIA_HIGHLIGHT_SUPPRESS_GetLastPerf(LIVE_HIGHLIGHT_SUPPRESS_GRP, &hs_perf) == 0) {
                if (hs_perf.gpu_enabled) {
                    snprintf(module_text, sizeof(module_text), "GPU %.3f/%.3fms",
                             hs_perf.gpu_kernel_ms, hs_perf.gpu_queue_ms);
                } else {
            ...
            printf("HIGHLIGHT_SUPPRESS frames=%d processed=%d low=%.2f high=%.2f ratio=%.2f strength=%.2f cpu=%.0f%% gpu=%s rga=%s module=%s\n",
                   frame, highlight_suppress_frames,
                   HIGHLIGHT_SUPPRESS_THRESHOLD_LOW, HIGHLIGHT_SUPPRESS_THRESHOLD_HIGH,
                   HIGHLIGHT_SUPPRESS_RATIO, HIGHLIGHT_SUPPRESS_STRENGTH,
                   g_perf.cpu_percent, gpu_text, rga_text, module_text);
        }
    ...
    unbind_vi_highlight_suppress_vmix_osd_vo(highlight_suppress_vi_bind_display_ok);
