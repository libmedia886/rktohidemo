...
static const char *g_module_pages[] = {
    ...
    "DCP_FAST_DEHAZE", "THERMAL", "THERMAL_LOWLIGHT_FUSION_CL", "THERMAL_SR_NPU", "DETECT_NPU", "FRUIT_DETECT_NPU", "HELMET_DETECT_NPU", "SEGMENT_NPU", "CONV_CL", "TRANSFORM", "BLEND_PYR", "EDOF_CL",
...
static const char *g_default_pages[] = {
    ...
    "CAP_DEHAZE", "CAP_DEHAZE_OFFLINE", "CONV_CL", "TRANSFORM", "THERMAL", "THERMAL_LOWLIGHT_FUSION_CL", "THERMAL_SR_NPU", "DETECT_NPU", "FRUIT_DETECT_NPU", "HELMET_DETECT_NPU", "SEGMENT_NPU",
...
static const page_desc_t g_page_descs[] = {
    ...
    {"FRUIT_DETECT_NPU",
     ...
     PAGE_BIND_NONE},
    {"HELMET_DETECT_NPU",
     "数据流：GDUT-HWD安全帽素材转H264 -> VDEC -> RGA -> DETECT_NPU(YOLOv8/RKNN) -> OSD -> VO。",
     "展示重点：检测是否佩戴安全帽，并区分blue、white、yellow、red四种安全帽和no_helmet类别。",
     0,
     PAGE_BIND_NONE},
    {"EIS_DETECT_NPU",
...
const char *canonical_tile_name(const char *name) {
    ...
    if (name && (strcasecmp(name, "HELMET_NPU") == 0 || strcasecmp(name, "HELMET_DETECT") == 0)) return "HELMET_DETECT_NPU";
