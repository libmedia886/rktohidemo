...
static void copy_npu_result(const MEDIA_NPU_RESULT *src, npu_result_snapshot_t *dst) {
    ...
    if (count < 0) count = 0;
    if (count > NPU_MAX_DRAW_OBJECTS) count = NPU_MAX_DRAW_OBJECTS;
    dst->object_count = count;
    for (int i = 0; i < count; ++i) {
        const MEDIA_NPU_OBJECT *obj = &src->u.detect.objects[i];
        npu_object_t *out = &dst->objects[i];
        out->x = obj->x;
        out->y = obj->y;
        out->w = obj->w;
        out->h = obj->h;
...
static int update_npu_overlay(uint64_t vdec_count, uint64_t pre_count,
    ...
    if (page_overlay_set_text(NPU_OSD_GRP, 1, 12, 560, 1,
                              "FLOW H264->VDEC->RGA->DETECT_NPU->RGA->OSD->VO",
...
int page_npu_run(volatile sig_atomic_t *running) {
    ...
    if (!path_readable(NPU_H264_PATH)) {
        fprintf(stderr, "DETECT_NPU demo H264 missing: %s\n", NPU_H264_PATH);
        return 1;
    }
    if (!path_readable(NPU_MODEL_PATH)) {
        fprintf(stderr, "DETECT_NPU demo model missing: %s\n", NPU_MODEL_PATH);
        return 1;
    }
    if (load_h264_stream(NPU_H264_PATH, &stream) != 0) {
        fprintf(stderr, "DETECT_NPU demo: failed to load H264 stream\n");
        return 1;
    }
    if (setup_npu_chain(&chain) != 0) {
        fprintf(stderr, "DETECT_NPU standalone chain setup failed\n");
    ...
    printf("DETECT_NPU standalone: H264(%s) -> VDEC%d.%s -> RGA%d.%s -> MEDIA_NPU%d.%s -> RGA%d.%s -> OSD%d.%s -> VO0.%s model=%s adapter=%s. Ctrl+C to stop.\n",
           NPU_H264_PATH,
    ...
    while (!running || *running) {
        ...
        if (feed_h264_stream_packet(NPU_VDEC_CHN, &stream, &is_vcl) != 0) {
            fprintf(stderr, "DETECT_NPU demo: MEDIA_VDEC_SendPacket failed\n");
        ...
        if (is_vcl) {
            ...
            if ((frame % NPU_FPS) == 0) {
                ...
                printf("DETECT_NPU vdec=%llu pre_rga=%llu npu=%llu post_rga=%llu osd=%llu vo=%llu det=%d frame=%d standalone=1",
                       (unsigned long long)vdec_count,
                       (unsigned long long)pre_count,
                       (unsigned long long)npu_count,
                       (unsigned long long)post_count,
