...
#define NPU_H264_PATH "assets/loop/helmet_detect/gdut_hwd_helmet_640x640.h264"
#define NPU_MODEL_PATH "assets/npu/gdut_hwd_helmet_yolov8n_640_fp.rknn"
#define NPU_LABEL_PATH "assets/npu/gdut_hwd_helmet_labels.txt"
#define NPU_MAX_DRAW_OBJECTS 8
...
static void helmet_rect_color(int class_id, uint8_t *r, uint8_t *g, uint8_t *b) {
    switch (class_id) {
    case 0: *r = 60; *g = 160; *b = 255; return;   /* blue_helmet */
    case 1: *r = 235; *g = 235; *b = 255; return;  /* white_helmet */
    case 2: *r = 255; *g = 220; *b = 40; return;   /* yellow_helmet */
    case 3: *r = 255; *g = 80; *b = 60; return;    /* red_helmet */
    case 4: *r = 255; *g = 40; *b = 40; return;    /* no_helmet */
    default: *r = 80; *g = 255; *b = 180; return;
    }
}
...
static int update_npu_box_regions(const npu_result_snapshot_t *det) {
    for (int i = 0; i < NPU_MAX_DRAW_OBJECTS; ++i) {
        ...
        helmet_rect_color(obj->class_id, &rect.color.r, &rect.color.g, &rect.color.b);
        rect.color.a = 220;
...
static int update_npu_overlay(uint64_t vdec_count, uint64_t pre_count,
    ...
    if (page_overlay_set_text(NPU_OSD_GRP, 0, 12, 12, 2,
                              "HELMET_DETECT_NPU GDUT-HWD YOLOV8 RKNN",
    ...
    if (page_overlay_set_text(NPU_OSD_GRP, 1, 12, 560, 1,
                              "FLOW GDUT_HWD_H264->VDEC->RGA->YOLOV8_NPU->RGA->OSD->VO",
...
static int setup_npu_chain(npu_chain_t *chain) {
    ...
    set_tile_status("HELMET_DETECT_NPU", TILE_LIVE);
...
int page_helmet_detect_npu_run(volatile sig_atomic_t *running) {
    ...
    if (!path_readable(NPU_H264_PATH)) {
        fprintf(stderr, "HELMET_DETECT_NPU demo H264 missing: %s\n", NPU_H264_PATH);
        return 1;
    }
    if (!path_readable(NPU_MODEL_PATH)) {
        fprintf(stderr, "HELMET_DETECT_NPU demo model missing: %s\n", NPU_MODEL_PATH);
        return 1;
    }
    if (load_h264_stream(NPU_H264_PATH, &stream) != 0) {
        fprintf(stderr, "HELMET_DETECT_NPU demo: failed to load H264 stream\n");
        return 1;
    }
    if (setup_npu_chain(&chain) != 0) {
        fprintf(stderr, "HELMET_DETECT_NPU standalone chain setup failed\n");
    ...
    printf("HELMET_DETECT_NPU standalone: H264(%s) -> VDEC%d.%s -> RGA%d.%s -> MEDIA_NPU%d.%s -> RGA%d.%s -> OSD%d.%s -> VO0.%s model=%s adapter=%s. Ctrl+C to stop.\n",
           NPU_H264_PATH,
    ...
    while (!running || *running) {
        ...
        if (feed_h264_stream_packet(NPU_VDEC_CHN, &stream, &is_vcl) != 0) {
            fprintf(stderr, "HELMET_DETECT_NPU demo: MEDIA_VDEC_SendPacket failed\n");
        ...
        if (is_vcl) {
            ...
            if ((frame % NPU_FPS) == 0) {
                ...
                printf("HELMET_DETECT_NPU vdec=%llu pre_rga=%llu npu=%llu post_rga=%llu osd=%llu vo=%llu det=%d frame=%d standalone=1",
                       (unsigned long long)vdec_count,
                       (unsigned long long)pre_count,
                       (unsigned long long)npu_count,
                       (unsigned long long)post_count,
